...
2. **🔄 Automated Data Reshaping & Nesting (`w2l_nest`, `c2p_nest`, `r2p_nest`)**
...
3. **🧪 Cross-Validation & Model Evaluation (`split_cv`, `nest_cv`)**
...
4. **📊 Batch Exporting for Breeding Software (`export_nest`, `export_list`)**
   - Automatically builds local folder hierarchies and batch-exports data based on nested groups or list structures (e.g., `tempdir()/Line/Breed/data.txt`), **providing seamless text-file preparation to bridge the gap with command-line driven breeding evaluation software like DMU and BLUPf90**.
5. **🛠️ Phenotypic Statistics & Preprocessing (`top_perc`, `format_digits`, `get_path_info`)**
